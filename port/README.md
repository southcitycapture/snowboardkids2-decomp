...
| The campaign end to end (every course, a save after each race) | **courses 0-10 and all three Cross games are won on the user's own save** (`progress [111111111114]`, 737,350 gold). Course 11, the Ice Land boss, is the last one standing and the credits are behind it. `docs/PLAN.md` has the campaign table |
...
| Course 11, the Ice Land boss (the last course, and the credits) | **not won.** It got a full census -- `docs/PLAN.md`, "Course 11: the Ice Land boss, in full". It is not the Jingle Town boss with a different model: it reads only `hitReactionState` 0x3D, so the frying pan the pilot leads with everywhere else is a **no-op** here (fifteen supplied pans took it from 10 heads to 9); it has two bars and thirteen heads, not ten; it is invulnerable for the whole of the damage animation each hit starts; and its collision node is 0x150000. All four are fixed and it now loses four heads a race. What stands between four and thirteen is not the pilot -- its aim, ammunition and trigger were each measured and two of the three changes made it worse. A lead gauge (`sbk-bosslead:`) says the boss is ahead on every sample and the gap **grows** all race, 150 units of `lapProgressRemaining` to 650, while our rider is 43% faster in a straight line and takes fourteen wall contacts to the boss's none. The rider takes a worse line than the boss and gets knocked off it -- the same wall courses 8, 9 and 10 each hit |
